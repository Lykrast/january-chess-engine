	...
	public final IMoveType create(JsonObject json, JsonDeserializationContext context) throws JsonParseException {
		...
		if (tmp != null) {
			...
			for (JsonElement elem : array) {
				if (elem.isJsonNull()) throw new JsonParseException("MoveCondition contains a null element in its true moves");
				movesTrue.add(context.deserialize(elem, IMoveType.class));
			}
		}
		...
		if (tmp != null) {
			JsonArray array = tmp.getAsJsonArray();
			for (JsonElement elem : array) {
				if (elem.isJsonNull()) throw new JsonParseException("MoveCondition contains a null element in its false moves");
				movesFalse.add(context.deserialize(elem, IMoveType.class));
			}
		}
