 ...
 *
 */
public class MoveListWrapperMagnetic extends MoveListWrapper {
	private Behavior friendly, enemy;
	private boolean affectRoyal;
	
	public MoveListWrapperMagnetic(Piece piece, IMoveList list, Behavior friendly, Behavior enemy, boolean affectRoyal) {
		super(piece, list);
	...
	/**
	 * Check if the position is empty or contains the moving piece (as it doesn't actually move while calculating the move).
	 */
	...
	/**
	 * Say if at the position is a valid target (in range and is non royal if the modifier doesn't allow royal).
	 */
	...
	/**
	 * Append the displacement if possible. Returns either the move or the added move that is now the deepest in the sequence.
	 */
