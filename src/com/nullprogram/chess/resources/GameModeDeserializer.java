...
public class GameModeDeserializer implements JsonDeserializer<GameMode> {
	...
	public GameMode deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
		...
		tmp = obj.get("lines");
		BoardLine[] lines = null;
		if (tmp == null) lines = new BoardLine[0];
		else {
			JsonArray array = tmp.getAsJsonArray();
			List<BoardLine> list = new ArrayList<>();
			for (JsonElement elem : array) {
				list.add(context.deserialize(elem, BoardLine.class));
			}
			lines = list.toArray(new BoardLine[list.size()]);
		}
		...
		if (tmp == null) {
			return new GameMode(JSONUtils.getMandatory(obj, "name").getAsString(),
					...
					JSONUtils.getMandatory(obj, "width").getAsInt(), JSONUtils.getMandatory(obj, "height").getAsInt(),
					piecesList.toArray(new PiecePlacement[piecesList.size()]), promotionsW, promotionsB, checkMultiple, lines);
		}
		// Options
		else {
			JsonArray array = tmp.getAsJsonArray();
